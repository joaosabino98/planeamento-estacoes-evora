...
- **Empregos paramétricos por urbanização**: cada tipo de densidade traz defaults de `empregos/ha` e mix de usos (5 categorias: comércio, serviços, educ./saúde, cultura/lazer, indústria), editáveis no modal (secção "Empregos e mix de usos"). Os empregos da urbanização entram no cartão de empregos e no índice de Shannon, prorated pela fração da urbanização dentro da isócrona de 5 min de cada estação. **POIs do OSM dentro do polígono são descartados** para evitar dupla contagem com a estimativa paramétrica.
...
- Toggle "Mostrar novas urbanizações no mapa": oculta visualmente os polígonos, etiquetas e anéis de difusão das novas urbanizações sem afetar cálculos de população ou empregos.
